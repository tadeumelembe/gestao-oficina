...
<script>
    $(document).ready(function() {
        ...
        $.ajax({
            ...
            url: "{{ asset('assets/js/car-brands.json')}}",
        }).then(function(data) {
            $("#marca").select2({
                placeholder: 'Selecione a marca...',
                data: $.map(data.results, function(item) {
                    return {
                        id: item.text,
                        text: item.text
                    }
...
<script type="text/javascript">
    ...
    $('#create_form').on('submit', function(event) {
        ...
        photo_car = $('#photo_car').val();

        proprietario_nome = $('#proprietario_nome').val();
        proprietario_telefone = $('#proprietario_telefone').val();
        proprietario_bi = $('#proprietario_bi').val();
        proprietario_email = $('#proprietario_email').val();
        //  alert(combustivel);

        var fd = new FormData();
        // alert(fd);
        var files = $('#photo_car')[0].files;
        fd.append('matricula',matricula);
        fd.append('marca',marca);
        fd.append('modelo',modelo);
        fd.append('notas',notas);
        fd.append('proprietario',proprietario);
        fd.append('tipo',tipo);
        fd.append('combustivel',combustivel);
        fd.append('anoFabrico',anoFabrico);
        fd.append('proprietario_nome',proprietario_nome);
        fd.append('proprietario_telefone',proprietario_telefone);
        fd.append('proprietario_bi',proprietario_bi);
        fd.append('proprietario_email',proprietario_email);

        // Check file selected or not

           fd.append('photo_car',files[0]);
